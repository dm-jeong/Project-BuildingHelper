...
public interface Vote_service // 서비스 인터페이스 정의
{
	List<Vote> get_all_votes_and_page1(int address, Criteria cri); // (category가 관리소인 경우)모든 투표를 가져오는 함수
	List<Vote> get_all_votes_and_page2(int address, Criteria cri); // (category가 세대원, 동대표인 경우)모든 투표를 가져오는 함수
